...
App.runUni = () => {
  ...
  const stratLabel = strat ? E.COVARIATES.find(c => c.id === strat)?.etiqueta : null;
  ...
  if (outcomes.length === 1) {
    ...
    if (strat) {
      ...
      App.renderUniChartStrat(od, trendStrat, E.COVARIATES.find(c => c.id === strat));
    } else {
...
App.renderUniComparativeChart = (outcomes, year, strat) => {
  ...
  const cd = strat ? E.COVARIATES.find(c => c.id === strat) : null;
  if (cd) {
