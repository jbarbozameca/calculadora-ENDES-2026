...
const NIVEL_LABEL_I18N = {
  en: { nino: "👶 Children <5", mujer: "♀ Women reproductive age",
        adulto_salud: "🧍 Adults · health", hogar: "🏠 Household" },
  es: { nino: "👶 Niños <5 años", mujer: "♀ Mujeres edad fértil",
        adulto_salud: "🧍 Adultos · salud", hogar: "🏠 Hogar" },
  pt: { nino: "👶 Crianças <5", mujer: "♀ Mulheres em idade reprodutiva",
        adulto_salud: "🧍 Adultos · saúde", hogar: "🏠 Domicílio" }
};
const NIVEL_LABEL = { ...NIVEL_LABEL_I18N.en };  // copia, NO referencia (para que Object.assign no mute el diccionario original)
...
function t(key, fallback) {
  const dict = window.ENDES?.I18N?.[CURRENT_LANG];
...
App.setLanguage = (lang) => {
  ...
  // Update NIVEL_LABEL
  Object.assign(NIVEL_LABEL, NIVEL_LABEL_I18N[lang]);
  // Replace static texts (data-i18n)
  document.querySelectorAll("[data-i18n]").forEach(el => {
    const key = el.dataset.i18n;
    const txt = t(key, el.textContent);
    el.textContent = txt;
  });
  // Replace placeholders (data-i18n-placeholder)
  ...
  document.title = t("app.title") + " · 1996–2024";
  try { localStorage.setItem("endes_lang", lang); } catch(e) {}
  // Re-render selectors and current analysis (so dynamic content also gets new strings)
  if (window.ENDES && App._initialized) {
    App._refillAll();
    // Re-ejecutar análisis actual
    const active = document.querySelector(".tab-content.active")?.id;
    try {
      if (active === "univariado") App.runUni();
      else if (active === "bivariado") App.runBi();
      else if (active === "multivariado") App.runMulti();
      else if (active === "explorador") App.runExp();
    } catch(e) { /* primera carga - no hay nada que re-renderizar */ }
  }
};

App._refillAll = () => {
  const E = window.ENDES;
  // Re-llenar selectores que dependen del idioma
  const yearSelects = ["uni-year","bi-year","multi-year","exp-year"];
  yearSelects.forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;
    const cur = el.value;
    App.fillYearSelect(el, id === "exp-year");
    if (cur) el.value = cur;
  });
  // strat selects
  ["uni-strat","exp-strat"].forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;
    const cur = el.value;
    App.fillStratSelect(el);
    if (cur) el.value = cur;
  });
  // Reconstruir multi-selects PRESERVANDO selecciones actuales
  const rebuildMS = (containerId, options, opts) => {
    const cont = document.getElementById(containerId);
    if (!cont) return;
    const selected = [...cont.querySelectorAll(".ms-option input:checked")].map(cb => cb.value);
    App.buildMultiSelect(cont, options, opts);
    selected.forEach(id => {
      const cb = cont.querySelector(`.ms-option input[value="${id}"]`);
      if (cb) { cb.checked = true; cb.dispatchEvent(new Event("change")); }
    });
  };
  const outcomeOpts = { groupKey:"nivel",
    groupOrder:["nino","mujer","adulto_salud","hogar"],
    groupLabels: NIVEL_LABEL, kind:"outcomes" };
  rebuildMS("uni-outcome-select", E.OUTCOMES, outcomeOpts);
  rebuildMS("bi-outcome-select", E.OUTCOMES, outcomeOpts);
  rebuildMS("multi-main-select", E.MAIN_VARS, {});
};
...
App.init = () => {
  try {
    const E = window.ENDES;
    if (!E) {
      const msg = "⚠️ data.js failed to load. Verify that index.html, data.js, and app.js are in the same folder.";
      console.error(msg);
      document.body.insertAdjacentHTML("afterbegin",
        '<div style="padding:1rem;background:#fee;color:#900;font-family:sans-serif;">' + msg + '</div>');
      return;
    }
    // Detect language: localStorage → URL ?lang= → browser → default 'en'
    let lang = "en";
    try {
      const saved = localStorage.getItem("endes_lang");
      const urlLang = new URLSearchParams(location.search).get("lang");
      const navLang = (navigator.language || "en").slice(0,2);
      lang = urlLang || saved || (["en","es","pt"].includes(navLang) ? navLang : "en");
    } catch(e) {}
    App.setLanguage(lang);
    ...
    App.bindButtons();
    try { App.runUni(); }
    catch (e) { console.warn("runUni initial failed:", e); }
...
App.fillYearSelect = (selectEl, withPool = true) => {
  ...
  if (withPool) {
    const opt = document.createElement("option");
    opt.value = "pool"; opt.textContent = t("explorer.all_years","All years (pool)");
...
App.fillStratSelect = (selectEl) => {
  ...
  optTotal.value = ""; optTotal.textContent = t("label.stratify_none","(none - national total)");
...
App.renderUniComparativeChart = (outcomes, year, strat) => {
  ...
  const binarios = outcomes.filter(o => o.tipo === "binario");
  const continuos = outcomes.filter(o => o.tipo === "continuo");
  const hasBoth = binarios.length > 0 && continuos.length > 0;

  // Helper para construir trazas
  const buildTraces = (outs, multiplier) => {
    if (cd) {
      return cd.niveles.map((lev, i) => {
        const data = outs.map(od => {
          const u = E.getUnivariate(od.id, year, lev);
          if (!u) return null;
          return { x: od.etiqueta.slice(0, 35) + (od.unit ? ` (${od.unit})` : ""),
                   y: u.valor * multiplier,
                   ic: [(u.valor - u.ic_inf) * multiplier, (u.ic_sup - u.valor) * multiplier] };
        }).filter(x => x);
        return {
  ...
  if (hasBoth) {
    const c = document.getElementById("uni-chart");
    c.innerHTML = '<div id="uni-chart-bin" style="height:300px"></div>' +
                  '<div id="uni-chart-cont" style="height:300px;margin-top:1rem"></div>' +
                  '<div class="note" style="margin-top:0.5rem">' +
                  '⚠️ Mezcla de binarios y continuos: se muestran en paneles separados.</div>';
    Plotly.newPlot("uni-chart-bin", buildTraces(binarios, 100), {
      title: `Binarios (%) · ${year}` + (cd ? ` por ${cd.etiqueta}` : ""),
      barmode: cd ? "group" : undefined,
      yaxis: { title: "% (IC95%)" }, xaxis: { tickangle: -30 },
      margin: { t: 40, b: 100 }
    }, { responsive: true, displaylogo: false });
    Plotly.newPlot("uni-chart-cont", buildTraces(continuos, 1), {
      title: `Continuos (medias) · ${year}` + (cd ? ` por ${cd.etiqueta}` : ""),
      barmode: cd ? "group" : undefined,
      yaxis: { title: "Media (IC95%)" }, xaxis: { tickangle: -30 },
      margin: { t: 40, b: 100 }
    }, { responsive: true, displaylogo: false });
  } else if (continuos.length > 0) {
    Plotly.newPlot("uni-chart", buildTraces(continuos, 1), {
      title: `Medias · ENDES ${year}` + (cd ? ` por ${cd.etiqueta}` : ""),
      barmode: cd ? "group" : undefined,
      yaxis: { title: `Media (IC95%) ${continuos[0].unit ? "· " + continuos[0].unit : ""}` },
      xaxis: { tickangle: -30 }, margin: { t: 50, b: 120 }
    }, { responsive: true, displaylogo: false });
  } else {
    Plotly.newPlot("uni-chart", buildTraces(binarios, 100), {
      title: `Prevalencias · ENDES ${year}` + (cd ? ` por ${cd.etiqueta}` : ""),
      barmode: cd ? "group" : undefined,
      yaxis: { title: "% (IC95%)" }, xaxis: { tickangle: -30 },
      margin: { t: 50, b: 120 }
    }, { responsive: true, displaylogo: false });
...
App._runBiMulti = (outcomes, cd, year) => {
  ...
  // Detectar tipo de outcomes seleccionados
  const binarios = outcomes.filter(o => o.tipo === "binario");
  const continuos = outcomes.filter(o => o.tipo === "continuo");
  const hasBoth = binarios.length > 0 && continuos.length > 0;

  // Helper: construir trazas para un subset de outcomes
  const buildTraces = (outs, multiplier) => {
    return cd.niveles.map((lev, i) => {
      const data = outs.map(od => {
        const bi = E.getBivariate(od.id, year, cd.id);
        ...
        if (!row) return null;
        return { x: od.etiqueta.slice(0, 30) + (od.unit ? ` (${od.unit})` : ""),
                 y: row.valor * multiplier,
                 errUp: (row.ic_sup - row.valor) * multiplier,
                 errDn: (row.valor - row.ic_inf) * multiplier };
      }).filter(x => x);
      return {
        x: data.map(d => d.x), y: data.map(d => d.y),
        name: lev, type: "bar",
  ...
  if (hasBoth) {
    // Tipos mixtos: usar dos gráficos apilados (binarios arriba, continuos abajo)
    const chartCont = document.getElementById("bi-chart");
    chartCont.innerHTML = '<div id="bi-chart-bin" style="height:300px"></div>' +
                          '<div id="bi-chart-cont" style="height:300px;margin-top:1rem"></div>' +
                          '<div class="note" style="margin-top:0.5rem">' +
                          '⚠️ Has mezclado outcomes binarios (%) y continuos (medias). ' +
                          'Se muestran en dos paneles separados para mantener escalas correctas.</div>';
    Plotly.newPlot("bi-chart-bin", buildTraces(binarios, 100), {
      title: `Binarios (%) por ${cd.etiqueta} · ENDES ${year}`,
      barmode: "group", yaxis: { title: "% (IC95%)" },
      xaxis: { tickangle: -30 }, margin: { t: 40, b: 100 }
    }, { responsive: true, displaylogo: false });
    Plotly.newPlot("bi-chart-cont", buildTraces(continuos, 1), {
      title: `Continuos (medias) por ${cd.etiqueta} · ENDES ${year}`,
      barmode: "group", yaxis: { title: "Media (IC95%)" },
      xaxis: { tickangle: -30 }, margin: { t: 40, b: 100 }
    }, { responsive: true, displaylogo: false });
  } else if (continuos.length > 0) {
    // Solo continuos
    Plotly.newPlot("bi-chart", buildTraces(continuos, 1), {
      title: `Medias por ${cd.etiqueta} · ENDES ${year}`,
      barmode: "group",
      yaxis: { title: `Media (IC95%) ${continuos[0].unit ? "· " + continuos[0].unit : ""}` },
      xaxis: { tickangle: -30 }, margin: { t: 50, b: 140 }
    }, { responsive: true, displaylogo: false });
  } else {
    // Solo binarios (caso original)
    Plotly.newPlot("bi-chart", buildTraces(binarios, 100), {
...
App.runExp = () => {
  ...
  const yearSel = document.getElementById("exp-year").value;
  const strat = document.getElementById("exp-strat").value;
  const vd = E.VARIABLE_CATALOG.find(v => v.variable === varId);
  if (!vd) return;

  const stratObj = strat ? (E.COVARIATES.find(c => c.id === strat) ||
                            E.MAIN_VARS.find(c => c.id === strat)) : null;
  ...
  document.getElementById("exp-subtitle").innerHTML =
    ...
    (stratObj ? ` · Estratificado por <strong>${stratObj.etiqueta}</strong>` : "") +
    ` <span class="badge badge-demo">DEMO</span>`;

  const isPool = yearSel === "pool" || !yearSel;
  const years = isPool ? E.YEARS.filter(y => y >= 2010) : [+yearSel];
  ...
  // Niveles reales de la variable categórica
  let varLevels = null;
  if (vd.tipo !== "continuo") {
    varLevels = E.getVarLevels(varId);
    if (!varLevels) {
      varLevels = vd.tipo === "binario" ? ["No", "Sí"] : ["Cat. 1","Cat. 2","Cat. 3"];
    }
  }
  const stratLevels = stratObj ? stratObj.niveles : ["Total"];

  const continuousBase = {
    "HW1": 28, "HW2": 140, "HW3": 880, "HW56": 112, "HW70": -90, "HW71": -50, "HW72": 40,
    "QS700": 640, "QS701": 1580, "QS900A": 121, "QS900B": 75,
    "V012": 30, "V525": 18.5, "V201": 2.3, "V212": 21.6, "V214": 5.2,
    "V218": 2.1, "V613": 2.5,
    "HV220": 49, "HV012": 4.2, "HV216": 2.4, "HV040": 1850, "HV108": 9.2, "HV105": 32,
    "M14": 7.4, "M19": 3180, "B11": 38
  };

  // Generar filas: año × estrato × (categoría o media)
  const rows = [];
  years.forEach(y => {
    stratLevels.forEach((sLev, sIdx) => {
      const seedBase = `${varId}:${y}:${strat || "_"}:${sLev}`;
      const r = seedRng(seedBase);
      if (vd.tipo === "continuo") {
        const base = continuousBase[varId] || 50;
        const sd = Math.abs(base) * 0.18 + 1;
        const n = Math.floor((2000 + r * 8000) / stratLevels.length);
        const se = sd / Math.sqrt(n);
        const yearShift = varId === "V201" ? -(y-2010)*0.025 :
                          varId === "V212" ? (y-2010)*0.05 :
                          varId === "QS700" ? (y-2014)*1.5 : 0;
        const stratShift = sIdx * sd * 0.15 * (sIdx % 2 === 0 ? -1 : 1);
        const m = base + (r - 0.5) * sd * 0.4 + yearShift + stratShift;
        rows.push({ anio: y, estrato: sLev,
                    n: n, media: m, ic_inf: m - 1.96*se, ic_sup: m + 1.96*se,
                    p25: m - 0.67*sd, p50: m, p75: m + 0.67*sd });
      } else {
        const k = varLevels.length;
        const n = Math.floor((2000 + r * 8000) / stratLevels.length);
        const rawProps = varLevels.map((_, j) => 0.1 + seedRng(seedBase + ":" + j));
        const sumP = rawProps.reduce((a,b) => a+b, 0);
        const props = rawProps.map(p => p / sumP);
        varLevels.forEach((cat, i) => {
          const p = props[i];
          const se = Math.sqrt(p * (1 - p) / n);
          rows.push({ anio: y, estrato: sLev, categoria: cat,
                      n: Math.round(n * p), prop: p,
                      ic_inf: Math.max(0, p - 1.96*se),
                      ic_sup: Math.min(1, p + 1.96*se) });
        });
      }
    });
  });
  ...
  if (vd.tipo === "continuo") {
    head.innerHTML = `<tr><th>Año</th>${stratObj ? `<th>${stratObj.etiqueta}</th>` : ""}
      <th class="numeric">N</th><th class="numeric">Media (IC95%)</th>
      <th class="numeric">P25</th><th class="numeric">P50</th><th class="numeric">P75</th></tr>`;
    body.innerHTML = rows.map(r =>
      `<tr><td>${r.anio}</td>${stratObj ? `<td>${r.estrato}</td>` : ""}
        <td class="numeric">${fmt.num(r.n)}</td>
    ...
    head.innerHTML = `<tr><th>Año</th>${stratObj ? `<th>${stratObj.etiqueta}</th>` : ""}
      <th>Categoría</th><th class="numeric">N</th><th class="numeric">% (IC95%)</th></tr>`;
    body.innerHTML = rows.map(r =>
      `<tr><td>${r.anio}</td>${stratObj ? `<td>${r.estrato}</td>` : ""}
        <td>${r.categoria}</td>
        <td class="numeric">${fmt.num(r.n)}</td>
  ...
  if (vd.tipo === "continuo") {
    if (isPool) {
      // Tendencia temporal con línea por estrato
      const traces = stratLevels.map((sLev, i) => {
        const data = years.map(y => rows.find(r => r.anio === y && r.estrato === sLev));
        return {
          x: years, y: data.map(d => d?.media ?? null),
          name: sLev, type: "scatter", mode: "lines+markers",
          line: { color: PALETTE[i % PALETTE.length], width: 2 }, marker: { size: 7 },
          error_y: { type: "data", symmetric: false,
                     array: data.map(d => d ? d.ic_sup - d.media : 0),
                     arrayminus: data.map(d => d ? d.media - d.ic_inf : 0) }
        };
      });
      Plotly.newPlot("exp-chart", traces, {
        title: `${vd.variable} · tendencia ${years[0]}-${years[years.length-1]}` +
               (stratObj ? ` por ${stratObj.etiqueta}` : ""),
        xaxis: { title: "Año" }, yaxis: { title: "Media (IC95%)" },
        margin: { t: 50 }, hovermode: "x unified"
      }, { responsive: true, displaylogo: false });
    } else {
      const data = rows.filter(r => r.anio === years[0]);
      Plotly.newPlot("exp-chart", [{
        x: data.map(r => r.estrato), y: data.map(r => r.media),
        type: "bar",
        marker: { color: data.map((_, i) => PALETTE[i % PALETTE.length]) },
        error_y: { type: "data", symmetric: false,
                   array: data.map(r => r.ic_sup - r.media),
                   arrayminus: data.map(r => r.media - r.ic_inf) }
      }], {
        title: `${vd.variable} · ${years[0]}` + (stratObj ? ` por ${stratObj.etiqueta}` : ""),
        yaxis: { title: "Media (IC95%)" }, margin: { t: 50, b: 80 },
        xaxis: { tickangle: stratLevels.length > 5 ? -30 : 0 }
      }, { responsive: true, displaylogo: false });
    }
  } else {
    // Categórica
    if (isPool && !stratObj) {
      // Tendencia: una línea por categoría
      const traces = varLevels.map((cat, i) => {
        const data = years.map(y => rows.find(r => r.anio === y && r.categoria === cat));
        return {
          x: years, y: data.map(d => d ? d.prop * 100 : null),
      ...
      Plotly.newPlot("exp-chart", traces, {
        title: `${vd.variable} · tendencia ${years[0]}-${years[years.length-1]}`,
        xaxis: { title: "Año" }, yaxis: { title: "% (IC95%)" },
        margin: { t: 50 }, hovermode: "x unified"
      }, { responsive: true, displaylogo: false });
    } else if (isPool && stratObj) {
      // Pool + estrato: mostrar primero y último año en gráficos comparativos
      const refYear = years[years.length-1];
      const traces = stratLevels.map((sLev, i) => {
        const data = rows.filter(r => r.anio === refYear && r.estrato === sLev);
        return {
          x: data.map(d => d.categoria), y: data.map(d => d.prop * 100),
          name: sLev, type: "bar",
          marker: { color: PALETTE[i % PALETTE.length] }
        };
      });
      Plotly.newPlot("exp-chart", traces, {
        title: `${vd.variable} · ${refYear} por ${stratObj.etiqueta} (último año)`,
        yaxis: { title: "%" }, barmode: "group",
        xaxis: { tickangle: varLevels.length > 4 ? -25 : 0 },
        margin: { t: 50, b: 100 }
      }, { responsive: true, displaylogo: false });
    } else if (!isPool && stratObj) {
      // Un año + estrato: barras agrupadas
      const traces = stratLevels.map((sLev, i) => {
        const data = rows.filter(r => r.anio === years[0] && r.estrato === sLev);
        return {
          x: data.map(d => d.categoria), y: data.map(d => d.prop * 100),
          name: sLev, type: "bar",
          marker: { color: PALETTE[i % PALETTE.length] }
      ...
      Plotly.newPlot("exp-chart", traces, {
        title: `${vd.variable} · ${years[0]} por ${stratObj.etiqueta}`,
        yaxis: { title: "%" }, barmode: "group",
        xaxis: { tickangle: varLevels.length > 4 ? -25 : 0 },
        margin: { t: 50, b: 100 }
      }, { responsive: true, displaylogo: false });
      ...
      const data = rows.filter(r => r.anio === years[0]);
      Plotly.newPlot("exp-chart", [{
        x: data.map(r => r.categoria), y: data.map(r => r.prop * 100),
        type: "bar",
        marker: { color: data.map((_, i) => PALETTE[i % PALETTE.length]) }
      }], {
        title: `${vd.variable} · distribución ${years[0]}`,
        yaxis: { title: "%" }, xaxis: { tickangle: varLevels.length > 4 ? -25 : 0 },
        margin: { t: 50, b: 100 }
      }, { responsive: true, displaylogo: false });
    }
  }
